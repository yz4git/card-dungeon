...
function renderBattle(){
  ...
  }).join('')}</div><div class="hand-label"><span>使用できるカード <b>${selectable.length}</b></span>${resolving?btn(`速度 ×${speed}`,'speed','text-button'):btn('配置をクリア','clear','text-button',filled?'':'disabled')}</div><div class="hand-scroll ${resolving?'playback-hand':''}"><div class="hand-grid">${selectable.map(c=>miniCard(c,{selected:b.plan.includes(c.uid),attrs:resolving?'disabled':'',extra:b.plan.includes(c.uid)?`<span class="assigned-badge">${base+b.plan.indexOf(c.uid)+1}手目</span>`:''})).join('')}</div>${resolving&&selectable.length===0?'<p class="empty-note">15枚すべてを使い切りました。</p>':''}</div><div class="battle-bottom"><div class="battle-hint">${resolving?`<span class="pulse-dot"></span> ${b.outcome?(b.outcome==='victory'?'勝利。カードを回収中…':'決着。持ち帰る1枚を選ぼう。'):'配置した順番で解決中'}`:filled===5?'5手の配置完了。タップすると入れ替えられます。':`空いている手にあと${5-filled}枚配置してください。`}</div>${btn(resolving?'5手を進行中…':`5手を実行 <span>${filled} / 5　→</span>`,'resolve','primary resolve-button',resolving||filled!==5?'disabled':'')}</div></div></section>`;
}
...
function continueGame(){game=JSON.parse(JSON.stringify(saved));slot=game.battle?.plan?.findIndex(x=>!x)??0;if(slot<0)slot=0;if(game.mode==='battle'&&game.battle.outcome){finishBattle(game);save();}render();world?.sync(game,true);if(game.mode==='battle'&&game.battle.phase==='resolve')scheduleTurn();audio.play('start');}
